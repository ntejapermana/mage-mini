                ...
                if($valueType=='arraySimple' || preg_match('/^ArrayOf/',$type)){
					...
					if(is_array($val) && count($val)> 0){
						foreach($val as $v){
							...
							$xml .= $this->serialize_val($v,'OutputDetailB2C',false,false,false,false,$use);
							++$i;
						}
	...
	function typeToForm($name,$type){
		...
		if($typeDef = $this->getTypeDef($type)){
			...
			// if array
				...
				for($i=0;$i < 3; $i++){
					$buffer .= "
					<tr><td align='right'>array OutputDetailB2C (type: $typeDef[arrayType]):</td>
		...
		if ($this->wsdl) {
			...
			if ($this->opData['style'] == 'rpc') {
				...
				if ($this->opData['output']['use'] == 'literal') {
					...
					if ($this->methodURI) {
						$payload = '<ns1:'.$this->methodname.' xmlns:ns1="'.$this->methodURI.'">'.$return_val.'</ns1:'.$this->methodname.">";
					} else {
						$payload = '<'.$this->methodname.'>'.$return_val.'</'.$this->methodname.'>';
					}
				} else {
					if ($this->methodURI) {
						$payload = '<ns1:'.$this->methodname.' xmlns:ns1="'.$this->methodURI.'">'.$return_val.'</ns1:'.$this->methodname.">";
	...
	function register($name,$in=array(),$out=array(),$namespace=false,$soapaction=false,$style=false,$use=false,$documentation='',$encodingStyle=''){
		...
		if(false == $soapaction) {
			if (isset($_SERVER)) {
				...
				$SERVER_NAME = $HTTP_SERVER_VARS['SERVER_ADDR'];
			...
			if (is_array($value) && sizeof($value) >= 1) {
				...
				foreach($value as $k => $v) {
					...
					if (!in_array($typeDef['arrayType'],$this->typemap['http://www.w3.org/2001/XMLSchema'])) {
					    $contents .= $this->serializeType('OutputDetailB2C', $typeDef['arrayType'], $v, $use);
					} else {
					    $contents .= $this->serialize_val($v, 'OutputDetailB2C', $typeDef['arrayType'], null, $this->XMLSchemaVersion, false, $use);
					} 
				}
			} else {
				$rows = 0;
	...
	*/
	function addOperation($name, $in = false, $out = false, $namespace = false, $soapaction = false, $style = 'rpc', $use = 'encoded', $documentation = '', $encodingStyle = ''){
		...
		if ($style == 'document') {
			...
			$this->addElement(array('name' => $name , 'type' => $name . 'ResponseType', 'form' => 'qualified'));
